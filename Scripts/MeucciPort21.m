...
tableJMHLY   = readtable([DATA_FOLDER    'JMHLY'   CSV_EXT]);
tableLVMUY   = readtable([DATA_FOLDER    'LVMUY'   CSV_EXT]);
tableNPSNY   = readtable([DATA_FOLDER    'NPSNY'   CSV_EXT]);
tableNSRGY   = readtable([DATA_FOLDER    'NSRGY'   CSV_EXT]);
tableSAP     = readtable([DATA_FOLDER    'SAP'     CSV_EXT]);
tableTOT     = readtable([DATA_FOLDER    'TOT'     CSV_EXT]);
tableTOYOF   = readtable([DATA_FOLDER    'TOYOF'   CSV_EXT]);
tableV       = readtable([DATA_FOLDER    'V'       CSV_EXT]);
...
% Constants
VAR_NAMES  = {  'ID'  ,  'Date'  ,  'BP' ,  'HSBC'  ,  'JMHLY',  'LVMUY' ,  'NPSNY'  ,  'NSRGY' ,  'SAP' ,  'TOT'   ,  'TOYOF' ,  'V' ,  'VIX'  };
...
tablePrice.(VAR_NAMES{5})  = tableJMHLY.Close;
tablePrice.(VAR_NAMES{6})  = tableLVMUY.Close;
tablePrice.(VAR_NAMES{7})  = tableNPSNY.Close;
tablePrice.(VAR_NAMES{8})  = tableNSRGY.Close;
tablePrice.(VAR_NAMES{9})  = tableSAP.Close;
tablePrice.(VAR_NAMES{10}) = tableTOT.Close;
tablePrice.(VAR_NAMES{11}) = tableTOYOF.Close;
tablePrice.(VAR_NAMES{12}) = tableV.Close;
...
prices = [tablePrice.BP tablePrice.HSBC tablePrice.JMHLY tablePrice.LVMUY   ...
          tablePrice.NPSNY tablePrice.NSRGY   tablePrice.SAP  tablePrice.TOT   ...
          tablePrice.TOYOF tablePrice.V   tablePrice.VIX];
...
subplot(4,1,1)
plot(optWindow,prices(1:WINDOW,NB_STOCKS+1));
grid on
title('Macro-economic indicator', 'fontsize', 12);
ylabel('VIX', 'fontsize', 12);
...
grid on
set(gca,'xlim',[min(optWindow) max(optWindow)])
title('Crisp State conditioning', 'fontsize', 12);
ylabel('Probability', 'fontsize', 12);
datetick('x','mmmyy','keeplimits','keepticks');

subplot(4,1,3)
semilogy(optWindow,k_p);
title('Kernel Smoothing conditioning', 'fontsize', 12);
ylabel('Probability', 'fontsize', 12);
grid on
set(gca,'xlim',[min(optWindow) max(optWindow)])
datetick('x','mmmyy','keeplimits','keepticks');
...
subplot(4,1,4)
area(optWindow,pPost);
title('Joint Time and State Conditioning', 'fontsize', 12);
ylabel('Probability', 'fontsize', 12);
grid on
set(gca,'xlim',[min(optWindow) max(optWindow)])
datetick('x','mmmyy','keeplimits','keepticks');
